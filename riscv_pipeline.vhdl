...
architecture Behavioral of riscv_pipeline is
    ...
    --signal ex_mem_mem_data, mem_wb_mem_data  : STD_LOGIC_VECTOR(31 downto 0);
...
--    rd  <= instr(11 downto 7);
    ...
    hazard_unit: hazard_detection_unit
        port map (
            reset => reset,
            if_id_mem_read => if_id_mem_read,--<what control signal?>,
            if_id_load_addr => if_id_load_addr,--<what control signal?>,
            instr    => instr,--<what signal?>,
            if_id_instr    => if_id_instr,--<what signal?>,
            if_id_rd       => if_id_rd,--<what signal?>,
...
--------------------------------------------------------------------------------
    ...
    reg_file_inst: reg_file
        port map (
            ...
            rs1       => if_id_rs1, --if_id_instr(19 downto 15),--
            rs2       => if_id_rs2, --if_id_instr(24 downto 20),--
            rd        => mem_wb_rd,--<what signal?>,
            data_in   => wb_data,--
            data_out1 => if_id_reg1_data,--<which_register>,
            data_out2 => if_id_reg2_data --<which_register>
        );    
...
-----------------------------------------------------------
    ...
    alu_input_b <= id_ex_imm when id_ex_alu_src = '1' else 
                   id_ex_reg2_data;
    --<which_register> when <which_control_signal> else
                   --<which_register>;
...
----------------------------------------------------------------------------------------
    ...
    next_pc <=  std_logic_vector(signed(ex_mem_npc) + signed(ex_mem_imm)) when ((ex_mem_branch = '1') and (not_equal_flag = '1')) else--<math based on NPC and imm> when (<what control signals?>) else -- branch case
                ...
                pc when (start_stall = '1') or (stall_counter > 0) else
                NPC;--pc when (<what control signals?>) else   -- stall case
